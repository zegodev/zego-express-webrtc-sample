...
$(async () => {
  ...
  $("#publish").click(() => {
    const result = zg.startPublishingStream(
      publishStreamID,
      previewStream ? previewStream : previewVideo.srcObject
    );
    published = true;
  ...
  $("#createRoom").click(async () => {
    ...
    $("#noiseSuppression").val() === "1"
      ...
      : (constraints.ANS = false);
    $("#autoGainControl").val() === "1"
      ? (constraints.AGC = true)
      : (constraints.AGC = false);
    $("#echoCancellation").val() === "1"
      ? (constraints.AEC = true)
      : (constraints.AEC = false);
    $("#audioBitrate").val() &&
      (constraints.audioBitrate = parseInt($("#audioBitrate").val()));
  ...
  $("#setAudioConfig").click(() => {
    ...
    zg.setAudioConfig(previewVideo.srcObject, constraints).then(
      (res) => {
  ...
  $("#mutePlayStreamVideo").click(() => {
    useLocalStreamList.forEach((item) => {
      zg.zegoWebRTC.mutePlayStreamVideo(
        item.streamID,
  ...
  $("#mutePlayStreamAudio").click(() => {
    useLocalStreamList.forEach((item) => {
      zg.zegoWebRTC.mutePlayStreamAudio(
        item.streamID,
        !$(this).hasClass("disabled")
      );
    });
